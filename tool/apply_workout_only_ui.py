...
w = workout.read_text()
w = re.sub(
    r"\n\s*TextField\(\n\s*controller: _deviceCalories,[\s\S]*?\n\s*\),\n",
    "\n",
    w,
    count=1,
)
w = re.sub(
    r"\n\s*Text\(\n\s*_deviceCalories\.text\.trim\(\)\.isNotEmpty[\s\S]*?\n\s*\),",
    "",
    w,
    count=1,
)
# Remove estimated kcal line if formatting differs.
w = re.sub(r"\n\s*Text\([^\n]*Estimated energy:[\s\S]*?\),", "", w, count=1)
